const fs = /*glsl*/ `
	...
	vec2 fold(vec2 inp, float n, float m, float sig, float drift, float _sdf){
	    vec2 v = inp;
	    for(float i = 0.; i < shift_n; i++){
	        float b=(i/shift_n)*(1.); //-comp
	        v-=b; //comp
	...
	void main(){
		...
		vec3 cta = texture2D(u_sampler, tuv+offs1).xyz;
		vec3 ctb = texture2D(u_sampler2, tuv+offs2).xyz;
		...
		vec3 ct1 = texture2D(u_sampler, uv_shift+offs1).xyz;
		vec3 ct2 = texture2D(u_sampler2, s_sig*uv_shift+offs2).xyz;

		vec3 ct1_r = texture2D(u_sampler3, uv_shift+offset(offs+offs_fine, vec2(10., 7.))).xyz;
		vec3 c_a = mix(ct1, ct1_r*(ct1*.5+.5), tex_attr*.6);

		vec3 c = 1.-mix(c_a, ct2, texmix);
		c *= satMat(1.+sat);

		gl_FragColor = contrastMat(1.+cont)*vec4(c, 1.0);
	}`;
